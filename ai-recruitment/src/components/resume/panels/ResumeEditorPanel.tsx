...
export function ResumeEditorPanel() {
    const { updatedContent: data, updateSection, improvements } = useResumeStore();
    ...
    return (
        <div className="w-full h-full flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl">
                        <Button
                            ...
                            className={cn("h-8 px-4 text-xs font-bold rounded-xl", !splitView ? "bg-white shadow-sm text-primary" : "text-slate-500")}
                        >
            ...
            <motion.div
                ...
                className="glass-panel flex-1 rounded-3xl p-6 lg:p-10 overflow-y-auto thin-scrollbar bg-white border shadow-xl relative min-w-0"
            >
                {splitView && (
                    <div className="absolute top-0 right-0 h-full w-1/2 bg-rose-50/50 border-l border-slate-200 pointer-events-none opacity-50 z-0"></div>
                )}

                {/* Contact Header */}
                <div className="text-center mb-10 relative z-10">
                    <h1 className="text-4xl font-black font-display tracking-tight text-slate-900 mb-3">
                        <EditableText
                            value={data.contactInfo.name}
                            onChange={v => handleUpdate("contactInfo", { ...data.contactInfo, name: v })}
                        />
                    </h1>
                    <div className="flex flex-wrap justify-center items-center gap-3 text-sm text-slate-600 font-medium tracking-wide">
                        ...
                        {data.experience.map((job, jIdx) => (
                            <div key={job.id} className="relative group">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className="text-xs uppercase font-bold tracking-widest text-slate-400 bg-slate-100 px-2 py-1 rounded-md">
                                        <EditableText
                                    ...
                                    {job.bullets.map((bullet, bIdx) => {
                                        ...
                                        return (
                                            <li key={bullet.id || bIdx} className="relative flex items-start gap-2 group/bullet">
                                                ...
                                                <div className={cn(
                                                    "text-sm text-slate-700 flex-1 leading-relaxed rounded-md px-1 transition-all duration-300",
                                                    hasFix ? "bg-amber-500/10 border border-amber-500/30" : "hover:bg-slate-50"
                                                )}>
                                                    <EditableText
                                                        value={bullet.text}
                                                        onChange={v => {
