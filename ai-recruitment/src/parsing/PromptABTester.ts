...
export class PromptABTester {
  ...
  /** Assign ExtractionPromptVariant (Phase 1) or legacy promptVariant. */
  static async assignVariant(
    resumeVersionId: string,
    tenantId?: string
  ): Promise<{ variantId: string; promptText: string | null; source: "v2" | "legacy" } | null> {
    if (tenantId) {
      const v2 = await PromptABTester.assignExtractionVariant(resumeVersionId, tenantId);
      if (v2) return v2;
    }
    const legacyId = await PromptABTester.assignLegacyVariant(resumeVersionId);
    if (!legacyId) return null;
    const variant = await prisma.promptVariant.findUnique({ where: { variantId: legacyId } });
    return {
      variantId: legacyId,
      promptText: variant?.promptText ?? null,
      source: "legacy",
    };
  }

  private static async assignExtractionVariant(
    resumeVersionId: string,
    tenantId: string
  ): Promise<{ variantId: string; promptText: string | null; source: "v2" } | null> {
    const existing = await prisma.extractionPromptAssignment.findFirst({
      where: { resumeVersionId },
      include: { promptVariant: true },
    });
    if (existing?.promptVariant) {
      return {
        variantId: existing.promptVariantId,
        promptText: existing.promptVariant.systemPrompt,
        source: "v2",
      };
    }

    const variants = await prisma.extractionPromptVariant.findMany({
      where: { tenantId, isActive: true, phase: 1 },
      orderBy: { createdAt: "asc" },
    });
    if (!variants.length) return null;

    const bucket = PromptABTester.hashBucket(resumeVersionId);
    const idx = bucket % variants.length;
    const chosen = variants[idx];

    await prisma.extractionPromptAssignment.create({
      data: {
        tenantId,
        resumeVersionId,
        promptVariantId: chosen.id,
      },
    });

    return {
      variantId: chosen.id,
      promptText: chosen.systemPrompt,
      source: "v2",
    };
  }

  private static async assignLegacyVariant(resumeId: string): Promise<string | null> {
    const existing = await prisma.promptAssignment.findUnique({ where: { resumeId } });
    ...
    await prisma.promptAssignment.create({
      data: { resumeId, variantId: chosen.variantId },
    });

    return chosen.variantId;
  }

  static async getPromptForResume(
    resumeId: string,
    tenantId?: string
  ): Promise<string | null> {
    const assigned = await PromptABTester.assignVariant(resumeId, tenantId);
    return assigned?.promptText ?? null;
  }

  /** @deprecated Use PromptABAnalyzer.analyze */
