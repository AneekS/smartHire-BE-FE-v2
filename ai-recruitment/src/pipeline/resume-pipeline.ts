...
import { runParseStage } from "@/pipeline/parse-stage";
import { runEmbedStageInline } from "@/pipeline/embed-stage";
...
export async function runResumePipeline(input: PipelineInput): Promise<PipelineResult> {
  ...
  const { indexed } = skipEmbed
    ? { indexed: 0 }
    : await runEmbedStageInline(draft.id, input.candidateId).catch((e) => {
